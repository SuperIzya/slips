...
trait Environment {
  ...
  object Node {
    ...
    object BuildStep {
      ...
      def modify(f: BuildContext => BuildContext): BuildStep[Unit] = State.modify(f)
    ...
    sealed trait BuildContext {
      val nodes: Nodes
      def add[T, N[x] <: Node[x]](node: N[T])(using Builder[N], TypeTest[Node[_], N[T]]): BuildStep[N[T]] = {
        nodes.collectFirst {
          case n: N[T] if n.signature == node.signature && node.equalNode(n) => n
        }
          .map(BuildStep.pure)
          .getOrElse(node.add)
    ...
    final case class Intake2[T1, T2](source1: Sink[T1], source2: Sink[T2]) extends Intake[T1 *: T2 *: EmptyTuple]


    sealed trait AlphaNode[X, Y] extends Node[Y]
    sealed trait BetaNode[X, Y, Z] extends Node[Z]
    final case class SourceNode[T](signature: String, f: ContextBuilder.Step[T]) extends Node[T] {
...
/*
  ...
  object Action {
    ...
    object InTuple {
      given [T <: Tuple, Q]: InTuple[Q *: T, Q] with
        override val pos = 0
      given [T <: NonEmptyTuple, Q](using prev: InTuple[Tuple.Tail[T], Q]): InTuple[T, Q] with
        override val pos = prev.pos + 1
    }
  ...
  object Rule {
    ...
    case class ThenBuilder[T <: Tuple] private[Environment] (name: String, condition: Condition[T]):
      def apply[Q](f: PartialFunction[Fact.Val[T], Action[T, Q]]): Rule[T, Q] = Rule(condition, f, name)
