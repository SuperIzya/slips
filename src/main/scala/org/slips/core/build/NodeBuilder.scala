...
object NodeBuilder {
  ...
  given [T]: NodeBuilder[Condition.Source[T]] = new NodeBuilder[Condition.Source[T]] {
    override def alphaNode(n: Condition.Source[T]): Option[BuildStep[AlphaNode]] = Some(BuildStep.getSourceNode(n))
  }

  given [T]: NodeBuilder[Predicate.Test[T]] = new NodeBuilder[Predicate.Test[T]] {
    override def alphaNode(n: Predicate.Test[T]): Option[BuildStep[AlphaNode]] = Option
